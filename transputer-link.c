...
ST_FUNC void relocate_plt(TCCState *s1)
{
    ...
    if (p < p_end) {
        ...
        if ((off + ((uint32_t)1 << 20)) >> 21)
            tcc_error("Failed relocating PLT (off=0x%llx, got=0x%llx, plt=0x%llx)", off, got, plt);
        write32le(p, 0x0);
        write32le(p + 4, 0x0);
        write32le(p + 8, 0x0);
        write32le(p + 12, 0x0);
        ...
        while (p < p_end) {
            ...
            if ((off + ((uint32_t)1 << 20)) >> 21)
                tcc_error("Failed relocating PLT (off=0x%llx, addr=0x%llx, pc=0x%llx)", off, addr, pc);
            write32le(p, 0x0);
            write32le(p + 4, 0x0);
            write32le(p + 8, 0x0);
            write32le(p + 12, 0x0);
...
void relocate(TCCState *s1, ElfW_Rel *rel, int type, unsigned char *ptr, addr_t addr, addr_t val)
{
    ...
    switch (type) {
        case R_TRANSPUTER_ABSOLUTE:
            {
                // Assuming the target is a 16-bit 'ldc' instruction.
                uint16_t encoded_val = 0x0100 | (val & 0xFF); // Encode the value into an 'ldc' instruction.
                write16le(ptr, encoded_val); // Write the instruction to the target location.
            }
            break;

        case R_TRANSPUTER_PC_RELATIVE:
            {
                // Calculate the difference between the symbol's address and the current instruction's address.
                int32_t relative_offset = (int32_t)(val - addr);

                // Assuming the target is a 16-bit 'ldc' instruction.
                uint16_t encoded_offset = 0x0100 | (relative_offset & 0xFF); // Encode the relative offset into an 'ldc' instruction.
                write16le(ptr, encoded_offset); // Write the instruction to the target location.
            }
            break;
